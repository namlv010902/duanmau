<?php
    ...
    if(isset($_POST["submit"])){
        if(empty($_POST["ten"])){
            $tenErr = "Bạn phải nhập tên !";
        }else{
            $ten = $_POST["ten"];
        }
        if(empty($_POST["sdt"])){
            $sdtErr = "Bạn phải nhập số điện thoại !";
        }else{
            $sdt = $_POST["sdt"];
        }
        if(empty($_POST["diachi"])){
            $diachiErr = "Bạn phải nhập địa chỉ !";
        }else{
            $diachi = $_POST["diachi"];
        }
...
<body>
    ...
    <header>
        ...
        <main>
            ...
            <div class="thongbao">
                <p style="font-size: 30px; font-weight: 500 ; color: green">
                    Mua hàng thành công. Bạn sẽ nhận được hàng trong thời gian sớm nhất  <br>
                  <strong style="color: darkorange"> Cảm ơn bạn đã tin tưởng mua hàng tại API</strong> 
                </p>
                <img src="https://vanchuyentrungquoc247.com/wp-content/uploads/2015/04/icon-3.png" width="30%" alt="">
                <a href="./lsmua.php" style="font-size: 25px; font-weight: 500;" >Đơn hàng của tôi </a>
            </div>
