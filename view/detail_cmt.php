...
<?php
   ...
   $query = " SELECT users.username, binhluan.noidung, binhluan.thoiGianBL,binhluan.id AS 'idBL', products.id AS 'idSP'
   FROM users JOIN binhluan ON users.id = binhluan.maKH 
   JOIN products ON products.id = binhluan.maSP WHERE products.id = $id";
    $detail_cmt= getAll($query);
...
<body>
    <div class="container">
        ...
        <tbody>
            <?php foreach($detail_cmt as $key => $value):?>
            <tr>
          <?php $_SESSION["idSP"]=$value["idSP"];
                 $_SESSION["idBL"]=$value["idBL"];
          ?>
                <td><?php echo $key + 1?></td>
                <td><?php echo $value["noidung"]?></td>
                <td><?php echo $value["thoiGianBL"]?></td>
                <td><?php echo $value["username"]?></td>
                <td id="action">

                   <a onclick="return confirm('Xóa comment này')" href="../controller/admin_delete_cmt.php?id=<?php echo $value["idBL"]?>"><i style="color:red" class="fas fa-trash-alt"></i></a>
                </td>
            </tr>
            <?php endforeach?>
        </tbody>
